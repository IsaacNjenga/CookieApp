...
function UpdateCookie({ modalContent, setOpenEditModal }) {
  ...
  const [values, setValues] = useState({
    ...
    allergen: modalContent.allergen || "",
    description: modalContent.description || "",
  });
  const [imageUrls, setImageUrls] = useState(modalContent.img || []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `update-cookie?id=${modalContent._id}`,
        updatedCookieData
      );
      if (res.data.success) {
        Swal.fire({
      ...
      Swal.fire({
        icon: "warning",
        title: "Error",
        text: "There was an error updating this cookie",
      });
    } finally {
      setLoading(false);
    }
